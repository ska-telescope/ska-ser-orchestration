...
output "instance_inventory" {
  ...
  value = {
    "${openstack_compute_instance_v2.instance.name}" = {
      ...
      ansible_ssh_private_key_file = "${data.openstack_compute_keypair_v2.keypair.name}.pem"
      ansible_ssh_common_args = join(" ", [
        "-o ControlPersist=30m",
        "-o StrictHostKeyChecking=no",
        "-o ProxyCommand=\"ssh -i \"${data.openstack_compute_instance_v2.jump_host.key_pair}.pem\" -W %h:%p -q ${local.jump_host_user}@${data.openstack_networking_floatingip_v2.jump_host_fip.address}\"",
        "-v"
      ])
      volumes = [
