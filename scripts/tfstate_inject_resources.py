...
"""This script injects resource ids in a Terraform state file
(pulled from GitLab) to manually alter the state
"""
...
def inject_ports(state_data: dict, ports_list: list[str]):
    """
    Injects port uuids into instance resources
    """
    for port in ports_list:
        ...
        for resource in state_data.get("resources", []):
            ...
            if (
                ...
                and resource_type == "openstack_compute_instance_v2"
                and resource_name == "instance"
            ):
                if "instances" in resource and len(resource["instances"]) == 1:
                    networks = resource["instances"][0]["attributes"].get(
                        "network", []
                    )
                    target_network_id = 0
...
if __name__ == "__main__":
    ...
    inject_ports(
        state,
        [port_map for port_map in args.ports.split(",") if port_map != ""],
    )
